...
public class StatTrackWrapper extends CustomEnchantment {
    ...
    public void onStatTrack(final StatTrackEvent e) {
        ...
        ItemStack tool = e.getContext().toolUsed();
        ...
        if (memoryLineIndex == -1) {
            ...
            currentLore.add(Utils.colorize("&8• &7Mined: &3" + formatNumberInK(memory + 1)));
        } else {
            currentLore.remove(memoryLineIndex);
            currentLore.add(memoryLineIndex, Utils.colorize("&8• &7Mined: &3" + formatNumberInK(memory + 1)));
        }
    ...
    private String formatNumberInK(int number) {
        if (number >= 1000) {
